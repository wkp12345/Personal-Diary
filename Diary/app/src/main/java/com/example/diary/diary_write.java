...
public class diary_write extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DbUtil();
        intent = getIntent();  //得到上一个页面传过来的值
        d_author.setText(intent.getStringExtra("author"));
        d_date.setText((new Date()).toString());
        back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //获取到两个输入框的值
                String title = d_title.getText().toString();
                String content = d_content.getText().toString();
                if(!title.equals("")){
                    diary d=new diary();
                    d.setTitle(title);
                    d.setContent(content);
                    d.setAuthor(d_author.getText().toString());
                    d.setDate(d_date.getText().toString());
                    d.setImage(iname);
                    dao.insert(d);
                }
                Intent intent = new Intent(diary_write.this,MainActivity.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP) ;  //FLAG_ACTIVITY_CLEAR_TOP也会清空目标Activity（A）和它上面的所有Activity，并且重新创建A，
                startActivity(intent);                             //给主页A设置launchMode为singleTask，这样A就不会重新创建了
            }
        });
        bt_image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                showPicturePicker(diary_write.this);
            }
        });
    }
    //图片
